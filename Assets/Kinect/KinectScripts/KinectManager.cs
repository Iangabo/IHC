...
public class KinectManager : MonoBehaviour
{
	...
	// None -> Sin suaavizado
	// Default -> Suavizado por defecto / Estandar
	// Medium -> Suavizado moderado
	// Aggressive -> Suavizado agresivo (Se ajusta mas rapido a los movimientos)
	public enum Smoothing : int { None, Default, Medium, Aggressive }

    // PUBLIC: Determina si se usan dos usuarios o no
    public bool TwoUsers = false;

    // Public Bool to determine if the sensor is used in near mode.
    // public bool NearMode = false;

    // PUBLIC: Determina si se usan los mapas de usuario
    public bool ComputeUserMap = false;

    // PUBLIC: Determina si se usan los mapas de color
	...
	public TextMeshPro GesturesDebugText;

    // Bool to keep track of whether Kinect has been initialized
    private bool KinectInitialized = false;
	public bool ShouldInitialized = false;
    ...
    // Retorna una UNICA instancia del kinectManager
    // USO: KinectManager kinect = KinectManager.Instance;
    ...
    // Verificar si el kinect esta inicializado
    /* USO:
	 * if(KinectManager.IsKinectInitialized()) {
	 *		// * Kinect listo para usar
	 *	}
	 *	else{
	 *		// * Error al inicializar el kinect
	 *	}
	*/
    ...
    // Verifica si el KinectManager esta listo para usar
    /* USO:
	 * if(IsInitialized()) {
	 *		// * Kinect listo para usar
	 *	}
	*/
	...
	// Funcion controlada por AvatarController
    ...
    // Retorna el mapa de profundidad de los usuarios
...
//				break;
    ...
    {
		while (!ShouldInitialized)
        {
			yield return null;
			Debug.Log("Waiting -> " + ShouldInitialized);
        }

		// Una vez que ShouldInitialized es true, puedes inicializar el Kinect
		InitializeKinect();
	}
	...
	private void InitializeKinect()
	{
		...
		try
		{
			...
			if (hr != 0)
			{
				throw new Exception("NuiInitiali" + "ze Failed");
			}
			...
			// [!] Crear instancia única del KinectManager y no destruir en cambio de escena
			instance = this;
    ...
    void OnGUI()
    {
		if(KinectInitialized)
		{
	        if(ComputeUserMap && (/**(allUsers.Count == 0) ||*/
    DisplayUserMap))
	        {
...
//				posJoint.x = KinectWrapper.Constants.ImageWidth - posJoint.x - 1;
	...
	// Método para desactivar el Kinect (por ejemplo, para el Profesor)
	/*
	void DisableKinect()
	{
		if (KinectInitialized)
		{
			// Aquí podrías realizar alguna lógica para cerrar el Kinect correctamente.
			KinectInitialized = false;
			Debug.Log("Kinect desactivado.");
		}
	}
	*/
